...
export default function AdminReservasPage(){
  ...
  return <main className="container"><section className="hero"><h1>Gerenciar reservas</h1><p>Confirme, inicie, finalize ou cancele locações.</p></section>{message&&<p className="message">{message}</p>}<section className="card">{items.length===0?<p>Nenhuma reserva cadastrada.</p>:items.map(r=><div className="rowItem" key={r.id}><strong>{r.vehicles?.name||'Veículo'} — {r.profiles?.full_name||'Cliente'}</strong><span>{new Date(r.pickup_at).toLocaleString('pt-BR')} → {new Date(r.return_at).toLocaleString('pt-BR')}</span><span>{r.days} diária(s) • R$ {Number(r.total).toFixed(2)}</span><label>Status<select value={r.status} onChange={e=>changeStatus(r.id,e.target.value)}>{statuses.map(s=><option value={s.value} key={s.value}>{s.label}</option>)}</select></label></div>)}</section><div className="actions section"><a className="btn secondary" href="/admin">Painel administrativo</a><a className="btn secondary" href="/admin/veiculos">Veículos</a></div></main>
}